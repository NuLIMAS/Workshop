...
#include "splitCyclicAMGInterface.H"
...
namespace Foam
{
    defineTypeNameAndDebug(splitCyclicAMGInterface, 0);
...
Foam::splitCyclicAMGInterface::splitCyclicAMGInterface
(
    ...
    coarseInterfaces_(coarseInterfaces)
{
    ...
    for (label ffi=0; ffi<localRestrictAddressing.size(); ffi++)
    {
        label curMaster = localRestrictAddressing[ffi];
        label curSlave = neighbourRestrictAddressing[ffi];
...
Foam::tmp<Foam::labelField> Foam::splitCyclicAMGInterface::transfer
(
...
Foam::tmp<Foam::labelField> Foam::splitCyclicAMGInterface::internalFieldTransfer
(
    const Pstream::commsTypes,
