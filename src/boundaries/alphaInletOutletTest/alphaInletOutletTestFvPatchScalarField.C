...
#include "alphaInletOutletTestFvPatchScalarField.H"
...
Foam::alphaInletOutletTestFvPatchScalarField::alphaInletOutletTestFvPatchScalarField
(
    const fvPatch& p,
...
:
    ...
    tempAlpha_(dict.lookupOrDefault<scalar>("tempAlpha", 0.0))
{
    // Read patch type
...
Foam::alphaInletOutletTestFvPatchScalarField::alphaInletOutletTestFvPatchScalarField
(
    const alphaInletOutletTestFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::alphaInletOutletTestFvPatchScalarField::updateCoeffs()
{
    if (this->updated())
    {
        return;
        ...
        forAll(phiAlpha,facei)
        {
            if (phiAlpha[facei] < 0)
            {
                scalar ai =  -s2/s1 + (tempAlpha_/c1) ;
                if (  ai < 0.63)
                {
                    alpha_new[facei] = ai;
                    this->refValue()[facei]= ai ;
                }
                else
                {
                    alpha_new[facei] = 0.63;
                    this->refValue()[facei]= 0.63;
                    tempAlpha_ = (-s2/s1 - 0.63)*c1;
                }
    ...
    if (!this->db().found(phiName_))
    {
        ...
        InfoIn
        (
            "void alphaInletOutletTestFvPatchField<Type>::"
            "updateCoeffs()"
...
void Foam::alphaInletOutletTestFvPatchScalarField::write(Ostream& os) const
{
    ...
    os.writeKeyword("tempAlpha") << tempAlpha_ << token::END_STATEMENT << nl;
}
...
/*
void Foam::alphaInletOutletTestFvPatchScalarField::operator=
(
...
namespace Foam
{
    ...
    (
        fvPatchScalarField,
        alphaInletOutletTestFvPatchScalarField
    );
