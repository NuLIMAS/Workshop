...
namespace Foam
{
    defineTypeNameAndDebug(interfaceFront, 0);
...
void Foam::interfaceFront::checkDefinition()
{
    if
    (
        !faceZoneID_.active()
    ...
    )
    {
        ...
        if (mesh.faceZones()[faceZoneID_.index()].size() == 0)
        {
            ...
            //     << "mesh definition."
            //     << abort(FatalError);
                Info <<  "interfaceFront zone contains no faces " << endl;
        }
    ...
    else
    {
        ...
        const primitivePatch& masterPatch =
            mesh.boundaryMesh()[masterPatchID_.index()];

        const primitivePatch& slavePatch =
        ...
        if (nSharedPoints > 0)
        {
            FatalErrorIn("void interfaceFront::checkDefinition()")
...
void Foam::interfaceFront::clearAddressing() const
{
...
Foam::interfaceFront::interfaceFront
(
    const word& name,
    const label index,
...
bool Foam::interfaceFront::setAttach() const
{
    if (!attached())
    {
        trigger_ = true;
...
bool Foam::interfaceFront::setDetach() const
{
    if (attached())
    {
        trigger_ = true;
...
bool Foam::interfaceFront::changeTopology() const
{
    if (manualTrigger())
    {
        if (debug)
        {
            Pout<< "bool interfaceFront::changeTopology() const "
                << " for object " << name() << " : "
                << "Manual trigger" << endl;
        }
...
void Foam::interfaceFront::setRefinement(polyTopoChange& ref) const
{
    ...
    if (trigger_)
    {
        ...
        else
        {
            FatalErrorIn
            (
                "void interfaceFront::setRefinement(polyTopoChange&) const"
...
void Foam::interfaceFront::updateMesh(const mapPolyMesh&)
{
...
void Foam::interfaceFront::write(Ostream& os) const
{
    os  << nl << type() << nl
        << name()<< nl
        << faceZoneID_.name() << nl
...
void Foam::interfaceFront::writeDict(Ostream& os) const
{
